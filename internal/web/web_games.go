...
func (h *Handler) handleGames(w http.ResponseWriter, r *http.Request) {
	...
	gRows, _ := h.DB.Query(`SELECT g.id, (SELECT name FROM engines WHERE id=g.black_id), (SELECT name FROM engines WHERE id=g.white_id), g.result, COALESCE(g.final_score,0), COALESCE(g.opening_line,''), COALESCE(g.created_at, unixepoch()) FROM games g ORDER BY g.id DESC LIMIT 100`)
	if gRows != nil {
		defer gRows.Close()
		for gRows.Next() {
			var id, s int
