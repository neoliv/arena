...
:root{--bg:#fafafa;--fg:#222;--muted:#666;--border:#ddd;--hover:#f0f0f5;--th-bg:#f0f0f0;--link:#36c;--link-hover:#25a;--win-bg:#dfd;--win-fg:#060;--loss-bg:#fdd;--loss-fg:#600;--draw-bg:#ffd;--draw-fg:#660;color-scheme:light}
@media(prefers-color-scheme:dark){:root{--bg:#1a1a2e;--fg:#e8e6e3;--muted:#a9a7a3;--border:#333;--hover:#252540;--th-bg:#22223a;--link:#7eb8ff;--link-hover:#a0cfff;--win-bg:#1a3a1a;--win-fg:#7f7;--loss-bg:#3a1a1a;--loss-fg:#f77;--draw-bg:#3a3a1a;--draw-fg:#ee7;color-scheme:dark}}
...
nav a{margin-right:1.5em;text-decoration:none;color:var(--link)}
...
func (h *Handler) handleGraphs(w http.ResponseWriter, r *http.Request) {
	...
	io.WriteString(w, pageHead+navHTML+`<h1>Graphs</h1>
		...
		<a href="?tab=elo" style="margin-right:1.5em;font-weight:`+func()string{if tab==""||tab=="elo"{return"bold"}else{return"normal"}}()+`">Elo Evolution</a>
		<a href="?tab=speed" style="margin-right:1.5em;font-weight:`+func()string{if tab=="speed"{return"bold"}else{return"normal"}}()+`">Speed</a>
		<a href="?tab=games" style="margin-right:1.5em;font-weight:`+func()string{if tab=="games"{return"bold"}else{return"normal"}}()+`">Games</a>
		<a href="?tab=timeout" style="margin-right:1.5em;font-weight:`+func()string{if tab=="timeout"{return"bold"}else{return"normal"}}()+`">Timeouts</a>
		<a href="?tab=performance" style="margin-right:1.5em;font-weight:`+func()string{if tab=="performance"{return"bold"}else{return"normal"}}()+`">Performance</a>
		</div>`)
...
func (h *Handler) handleMatches(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	io.WriteString(w, pageHead+navHTML+searchJS+`<h1>Matches</h1>`)
		...
		if aRows != nil { defer aRows.Close(); for aRows.Next() { var id, games int; var e1, e2, tc, started string; aRows.Scan(&id, &e1, &e2, &tc, &games, &started); fmt.Fprintf(w, `<tr class="filter-row"><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%d</td><td>%s</td></tr>`, id, e1, e2, tc, games, started[:min(19,len(started))]) } }

		// Completed
		io.WriteString(w, filterBox+`<h2>Completed</h2><table><tr><th>ID</th><th>Engine 1</th><th>Engine 2</th><th>Score</th><th>Games</th><th>Date</th></tr>`)
...
func (h *Handler) handleGames(w http.ResponseWriter, r *http.Request) {
	...
	// In-progress matches
	io.WriteString(w, `<h2>In Progress</h2><table><tr><th>ID</th><th>Engine 1</th><th>Engine 2</th><th>Time</th><th>Games</th><th>Started</th></tr>`)
	rows, _ := h.DB.Query(`SELECT a.id, (SELECT name||' '||version FROM engines WHERE id=a.engine1_id), (SELECT name||' '||version FROM engines WHERE id=a.engine2_id), COALESCE(a.time_control,'{}'), a.num_games, COALESCE(a.in_progress_at, a.created_at) FROM match_assignments a WHERE a.status='in_progress' ORDER BY a.id DESC LIMIT 20`)
	if rows != nil {
		defer rows.Close()
		for rows.Next() {
			var id, games int; var e1, e2, tc, started string
			rows.Scan(&id, &e1, &e2, &tc, &games, &started)
			fmt.Fprintf(w, `<tr class="filter-row"><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%d</td><td>%s</td></tr>`, id, e1, e2, tc, games, started[:min(19,len(started))])
		}
	}
	if rows == nil { io.WriteString(w, `<tr><td colspan="6">None</td></tr>`) }
	io.WriteString(w, "</table>")
